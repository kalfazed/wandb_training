"""Shared LightningModule / LightningDataModule / Dataset for ex07.

Both ``train.py`` and ``predict.py`` import from this file. Once a project
has more than one entry point, factoring the "what is a Module / DataModule"
into a shared module is the natural next step.

Nothing here is new vs ex03–ex06 EXCEPT for ``LitBEVDetector.predict_step``,
which makes the Module work with ``trainer.predict(...)``.
"""
...
class LitBEVDataModule(pl.LightningDataModule):
    def __init__(
        ...
        self._predict_ds: NuScenesBEVDataset | None = None
    ...
    def setup(self, stage: str | None = None) -> None:
        ...
        # For `predict`, expose ALL frames (no random subsample) so the user
        # can run inference on the whole dataset, not just the val slice.
...
# ---------------------------------------------------------------------------
# LightningModule — with a new `predict_step` for trainer.predict(...)
# ---------------------------------------------------------------------------
class LitBEVDetector(pl.LightningModule):
    def __init__(
        ...
        super().__init__()
        # save_hyperparameters() is what makes `LitBEVDetector.load_from_checkpoint`
        # work later: every __init__ kwarg is stored inside the .ckpt file, so
        # loading the model doesn't require remembering how to construct it.
    ...
    # -----------------------------------------------------------------------
    # NEW in ex07: predict_step is what `trainer.predict(...)` calls.
    # No backward, no loss, no logging — just return what you want collected.
    # The return value is auto-gathered across batches/DDP and returned by
    # ``trainer.predict()`` as a Python list (one element per batch).
    # -----------------------------------------------------------------------
